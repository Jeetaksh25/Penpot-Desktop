...
async function syncDirs(originPath, destPath) {
  // Cross-platform replacement for `rsync -ar --delete` so the desktop
  // build works on Windows where rsync is not on cmd.exe's PATH.
  const fs = await import("node:fs/promises");
  await fs.rm(destPath, { recursive: true, force: true });
  await fs.cp(originPath, destPath, { recursive: true, force: true });
}
