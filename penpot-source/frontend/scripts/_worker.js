...
function configureModulesProcessor(options) {
  ...
  return modulesProcessor({
    ...
    generateScopedName: (selector, filename, css) => {
      ...
      // Split on BOTH "/" and "\": on Windows ph.dirname returns a backslash
      // path (D:\...\src\app\main\ui). Splitting on "/" alone leaves the whole
      // absolute path as a single segment, so findIndex("app") returns -1 and
      // the entire path — with "\" (CSS escape char) and ":" (drive letter) —
      // becomes the class-name prefix, producing invalid CSS the browser can't
      // parse past (~first 250 rules), rendering the whole app unstyled.
      // Splitting on both separators yields the same valid "main_ui_..." prefix
      // on Windows that Linux/CI produces, so CSS and JS class names match.
